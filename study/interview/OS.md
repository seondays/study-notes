...
#### 💫 인터럽트 우선순위를 판별하는 방법에 대해 좀 더 구체적으로 설명해주세요
- 소프트웨어적 방법
	- 발생한 인터럽트들을 검사하여 우선순위가 높은 인터럽트를 찾아내는 방식
	- 우선순위 조절에 융통성이 있지만, 인터럽트가 많은 경우 처리 속도가 느려짐
- 하드웨어적 방법
	- 데이지 체인 : 인터럽트가 발생하는 장치들을 직렬로 버스에 연결하는데, 이 때 우선순위가 높은 장치들부터 앞쪽에 위치시키는 방식을 사용
	- 병렬 우선순위 : 각 장치들이 병렬로 연결되어 있으며 각자 우선순위를 가지고 있다. 동시에 인터럽트가 발생하는 경우, 낮은 우선순위를 가지는 장치를 마스킹하여 일시적으로 무시하는 방법을 사용

### 인터럽트 벡터 테이블이란?
인터럽트 벡터 테이블은 여러 인터럽트 번호에 해당하는 ISR의 시작 주소(인터럽트 벡터)가 저장되어 있는 테이블입니다.

인터럽트 요청이 발생하였을 때 CPU는 발생한 인터럽트 소스가 무엇인지, 그리고 해당 인터럽트 ISR이 어디에 적재되어 있는지를 확인 후 실행해야 하는데, 이 때 인터럽트 벡터 테이블을 이용합니다.

#### 💫 ISR란? 
인터럽트 서비스 루틴이란 인터럽트를 처리할 때 실행되는 코드 블록입니다. 인터럽트가 발생하면 CPU는 현재 수행 중인 작업을 멈추고 ISR를 실행하게 됩니다.
...
스레드의 주소공간은 스택과 PC 값을 포함하는 레지스터의 값, 스레드별로 가지는 저장공간인 TLS 등으로 구성되어 있습니다.
...
#### cpu 스케줄링에서 응답 시간, 대기 시간, 반환 시간이란?
- 응답 시간이란 사용자가 요청을 한 후 시스템이 처음으로 응답을 하는데까지 걸리는 시간입니다
- 대기 시간이란 프로세스가 준비 큐에서 CPU 할당을 기다리면서 대기한 총 시간
- 반환 시간이란 프로세스가 실행을 완료하고 종료할 때까지 걸린 . 총시간
...
#### 💫 동시성 관련 용어 정리
1. 동시성 : 여러 개의 프로세스나 스레드가 동시에 실행되는 것처럼 보이는 것을 의미합니다. 실제로 동시에 실행될 수도 있고, 마치 동시에 실행되는 것처럼 보이기도 하는 두 가지 경우가 있습니다.
2. 레이스 컨디션 (경합 상태) : 여러 개의 스레드가 공유 자원에 동시에 접근하여 접근 순서에 따라 결과값이 달라지는 상황을 의미합니다.
3. 임계 영역 (크리티컬 색션) : 여러 개의 스레드가 공유 자원에 접근하는 코드 영역을 의미합니다. 임계 영역에서 레이스 컨디션이 발생하게 됩니다.
4. 상호 배제 : 한 스레드가 임계 영역에 접근 중일 때, 다른 스레드는 접근하지 못하도록 막는 것을 의미합니다.
...
#### 💫 은행원 알고리즘
운영체제에서 교착 상태를 회피하기 위한 기법 중 하나로, 프로세스들이 자원을 요청할 때, 해당 자원을 할당해주어도 시스템이 교착 상태에 빠지지 않고 모든 프로세스가 성공적으로 완료될 수 있는 '안전 상태를 유지하는지 검사합니다.
...
### 캐시는 어떤 자료구조를 통해 일반적으로 구현된다고 할 수 있을까요?
캐시는 빠르게 해당되는 값을 조회해야 하기 때문에, 일반적으로 해시맵 구조로 값을 관리합니다. 그런데 캐시가 가득 차서 적당한 방법으로 이것을 제거해야 할 때 적용할 기준인 제거 정책의 구현이 필요합니다.

예를 들어 LRU 캐시라면 가장 사용한지 오래된 값을 제거해야 하므로, 값을 사용한 순서를 추적하는 자료구조가 필요합니다. 이 때 링크드리스트를 사용하여 최근 사용된 값을 head 위치에 추가하는 방식으로 관리할 수 있습니다.
자연스럽게 가장 오래 사용되지 않은 값은 tail에 존재하게 되어, 필요 시 해당 값을 우선적으로 제거합니다.

일반 리스트가 아닌 링크드 리스트를 사용하는 이유는 값의 삽입과 삭제가 빈번하기 때문입니다.
...
- 요구 페이징 기법으로 필요한 페이지만 올려서 사용하기 때문
...
그런데 반대로 물리 메모리에 올라갈 수 있는 페이지 수는 줄어들기 때문에, 공간 지역성이 낮은 경우에는 오히려 페이지 교체가 빈번하게 필요해져 페이지 폴트가 많이 발생할수도 있습니다.
...
#### 단편화란?
가상 메모리에서 메모리가 잘게 나뉘는 등의 이유로 효율적으로 사용할 수 없게 되는 현상을 의미합니다.
...
이 매핑 주소는 페이지 번호와 프레임 번호, offset을 활용하여 계산합니다.
...
## 19. TLB는 무엇인가요?
TBL는 가상 메모리 주소를 물리 메모리 주소로 변환하는 정보인 페이지 테이블 엔트리 정보를 저장하는 캐시로, MMU 내부에 존재합니다.
